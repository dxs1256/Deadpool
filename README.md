...
- **`Baidu.txt`**: 可访问百度（国内业务/回# 🕸️ SOCKS5 Proxy Checker
...
2. GitHub Actions 自动运行
机制: 每天 UTC 0:00 (北京时间 8:00) 自动执行。
结果: 检测完成后的新列表会自动 Commit 并 Push 到当前仓库。
手动触发: Actions -> Proxy Checker -> Run workflow。
⚙️ 配置
修改 config.toml 文件：
check_urls: 定义分类规则（包含 "baidu" 或 "google" 关键字）。
max_concurrent_req: 并发数（默认 200）。
